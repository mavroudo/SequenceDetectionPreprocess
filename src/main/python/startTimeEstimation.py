...
'''
    ...
    The start time of an activity instance (task) is defined as:
        *   the pre-defined trace start time, if the task is first in the trace.
        *   the end time of the last previous task in the trace, if the task is not first in the trace and that last previous task is not concurrent with it.
    ...
    with open(file_name, 'r') as reader:
        for line in reader:
            ...
            for i, task in enumerate(tasks):
                ...
                traces[trace_index].append(Task(type, end))
...
def estimate_start_times(traces, concurrents):
    for trace_index, tasks in traces.items():
        for i, task in enumerate(tasks):
...
if __name__ == '__main__':
    ...
    # Update the traces by estimating the start time of the tasks
    estimate_start_times(traces, concurrents)
