...
public class XLogConverter extends XConverter {
	...
	public void marshal(Object obj, HierarchicalStreamWriter writer,
		...
		context.convertAnother(log.getAttributes());
		writer.endNode();
		for(XTrace trace : log) {
			writer.startNode("XTrace");
			context.convertAnother(trace);
			writer.endNode();
		}
	}
	...
	public Object unmarshal(HierarchicalStreamReader reader,
		...
		reader.moveDown();
		XAttributeMap attributes = (XAttributeMap)context.convertAnother(log, XAttributeMap.class);
		log.setAttributes(attributes);
		reader.moveUp();
		while(reader.hasMoreChildren()) {
			reader.moveDown();
			XTrace trace = (XTrace)context.convertAnother(log, XTrace.class);
			log.add(trace);
			reader.moveUp();
		}
		return log;
