...
/**
 * The extension manager is used to access, store, and manage
 * extensions in a system. Extensions can be loaded from their
 * given URI, which should point to the file defining the extension.
 * Also, extensions can be registered locally, which then override
 * any remotely-loaded extensions (which are more generic placeholders).
 * 
 * Extension files downloaded from remote sources (which happens
 * when the extension cannot be resolved locally) are cached on the
 * local system, so that the network source of extension files is not
 * put under extensive stress.
 * 
 * The extension manager is a singleton, there is no need to
 * instantiate more than one extension manager, which is necessary
 * to avoid states of inconsistency.
 * 
...
public class XExtensionManager {
	...
	/**
	 * Maximal time for caching remotely-defined extensions in
	 * milliseconds. The default for this value is one day (24 hrs.)
	 */
	...
	/**
	 * Map storing all extensions currently registered, indexed
	 * by their unique URI.
	 */
	private HashMap<URI,XExtension> extensionMap;
	/**
	 * List mapping each extension currently registered to a
	 * unique index.
	 */
	...
	/**
	 * Explicitly registers an extension instance with the
	 * extension manager.
	 * 
	 ...
	 * If the extension has not been registered before, it
	 * is looked up in the local cache. If it cannot be found
	 * in the cache, the manager attempts to download it
	 * from its unique URI, and add it to the set of managed
	 * extensions.
	 * 
	...
	public XExtension getByUri(URI uri) {
		...
		if(extension == null) {
			try {
				...
				register(extension);
				XLogging.log("Imported XES extension '" + extension.getUri() + "' from remote source", XLogging.Importance.DEBUG);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}
	...
	/**
	 * Retrieves an extension by its name. If no extension by
	 * that name can be found, this method returns <code>null</code>.
	 * 
	 * @param name The name of the requested extension.
	 * @return The requested extension (may be <code>null</code>,
	...
	/**
	 * Retrieves an extension by its prefix. If no extension by
	 * that prefix can be found, this method returns <code>null</code>.
	 * 
	...
	/**
	 * Retrieves an extension by ints index. If no extension with
	 * the given index is found, this method returns <code>null</code>.
	 * 
	...
	/**
	 * Resolves the index of an extension, given that this extension
	 * has been previously registered with this manager instance. If
	 * the given index has not been registered previously, this method
	 * returns <code>-1</code>.
	 * 
	...
	/**
	 * Registers all defined standard extensions with 
	 * the extension manager before caching.
	 */
	...
	/**
	 * Downloads and caches an extension from its remote
	 * definition file. The extension is subsequently
	 * placed in the local cache, so that future loading
	 * is accelerated.
	 * 
	 * @param uri Unique URI of the extension which is to
	 * be cached.
	 */
	 ...
	 * Cached extensions which exceed the maximum caching age
	 * are discarded, and downloaded freshly.
	 */
	protected void loadExtensionCache() {
		...
		if(extFiles == null) {
			// Extension folder may be non-existant for virtual users with no home directory
			...
			if(extFile.lastModified() < minModified) {
				...
				try {
					...
					if(extensionMap.containsKey((extension).getUri()) == false) {
						extensionMap.put(extension.getUri(), extension);
						extensionList.add(extension);
						XLogging.log("Loaded XES extension '" + extension.getUri() + "' from cache", XLogging.Importance.DEBUG);
					} else {
						XLogging.log("Skipping cached XES extension '" + extension.getUri() + "' (already defined)", XLogging.Importance.DEBUG);
					}
				} catch(Exception e) {
