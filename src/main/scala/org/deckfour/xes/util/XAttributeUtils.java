...
public class XAttributeUtils {
	...
	/**
	 * For the given attribute, returns its type, i.e., the most
	 * high-level, typed interface this attribute implements.
	 * 
	...
	/**
	 * For the given attribute, derives the standardized string
	 * describing the attributes specific type (used, e.g., for
	 * serialization).
	 * 
	 * @param attribute Attribute to extract type string from.
	 * @return String representation of the attribute's specific
	 * type.
	 */
	...
	/**
	 * Derives a prototype for the given attribute. This prototype
	 * attribute will be equal in all respects, expect for the value
	 * of the attribute. This value will be set to a default value,
	 * depending on the specific type of the given attribute.
	 * 
	...
	/**
	 * Composes the appropriate attribute type from
	 * the string-based information found, e.g., in XML serializations.
	 * 
	 * @param factory Factory to use for creating the attribute.
	 * @param key Key of the attribute.
	 * @param value Value of the attribute.
	 * @param type Type string of the attribute.
	 * @param extension Extension of the attribute (can be <code>null</code>).
	...
	public static XAttribute composeAttribute(XFactory factory, String key, String value, String type, XExtension extension) {
		...
		if(type.equalsIgnoreCase("LITERAL")) {
			...
			XAttributeBoolean attr = factory.createAttributeBoolean(key, Boolean.parseBoolean(value), extension);
			return attr;
		} else if(type.equalsIgnoreCase("CONTINUOUS")) {
			XAttributeContinuous attr = factory.createAttributeContinuous(key, Double.parseDouble(value), extension);
			return attr;
		} else if(type.equalsIgnoreCase("DISCRETE")) {
			XAttributeDiscrete attr = factory.createAttributeDiscrete(key, Long.parseLong(value), extension);
			return attr;
		} else if(type.equalsIgnoreCase("TIMESTAMP")) {
			XAttributeTimestamp attr;
			try {
				attr = factory.createAttributeTimestamp(key, DateFormat.getDateTimeInstance().parse(value), extension);
	...
	/**
	 * Static helper method for extracting all extensions
	 * from an attribute map.
	 * 
	 * @param attributeMap The attribute map from which to
	 * extract extensions.
