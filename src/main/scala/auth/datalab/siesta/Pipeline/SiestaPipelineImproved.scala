...
object SiestaPipelineImproved {

  def execute(c: Config): Unit = {

    val dbConnector = new S3ConnectorSimple()
    ...
    spark.time({
      ...
      //TODO: remove intervals i dont think we need them (we can use something similar to last_checked)
      val intervals = Intervals.intervals(sequenceRDD, metadata.last_interval, metadata.split_every_days, min_ts, max_ts)
      ...
      //extract new pairs
      val pairs = ExtractPairsSimple.extract(inverted, lastChecked, intervals, metadata.lookback)
