  ...
  def getIterations(c: Config, data: RDD[Structs.Sequence], traces: Int): Iterations = {
    ...
    val iterations: Int = { // determines iterations
      if (c.iterations != -1) c.iterations
        ...
        try {
          partitionNumber = if (minExecutorMemory / size_estimate_trace > traces) 1 else ((size_estimate_trace * traces) / minExecutorMemory).toInt + 1
        } catch {
          case _: Exception =>
            Logger.getRootLogger.log(Level.INFO, "Too small dataset, setting to one")
        }
